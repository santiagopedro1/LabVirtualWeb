  ...
  tooltip: {
    ...
    formatter: (data: any) => {
      ...
      data.forEach((item: any, index: number) => {
        let formattedValue = Object.values(item.value)[(index % 3) + 1] as any
        if (isNaN(formattedValue)) formattedValue = 'Sem leitura'
        if (item.seriesName.includes('Temperatura')) formattedValue += 'ºC'
        else if (item.seriesName.includes('Umidade')) formattedValue += '%'
